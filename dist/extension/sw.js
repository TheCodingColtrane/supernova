    ...
    (function(global2, factory) {
      ...
      function createConnectionsManager() {
        if (typeof FinalizationRegistry !== "undefined" && typeof WeakRef !== "undefined") {
          ...
          return { toArray, add: add2, remove: remove2 };
        } else {
          var connections_1 = [];
          var toArray = function() {
...
async function getWeekLawsuitsData() {
  try {
...
async function getWeekLawsuits(considerHoliday = false) {
  try {
    ...
    if (considerHoliday) {
      ...
      const hasHolidays = data.some(
        (x) => new Date(x.initialDeadline).getFullYear() < year || new Date(x.deadline).getFullYear() < year
      );
      if (hasHolidays) {
...
async function saveLawsuits(lawsuits) {
  try {
